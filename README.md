...
|---|---|---|
| 2026-08-22 | `RX_Master` | **ปรับปรุง Telegram ให้แจ้งเตือนทันทีเมื่อสถานะระดับน้ำเปลี่ยน พร้อมระบบ Priority Queue:**<br>1. **Instant State-Change Alert:** เมื่อสถานะระดับน้ำเปลี่ยน (Safe→Warning, Warning→Critical, Critical→Safe ฯลฯ) ส่ง Telegram แจ้งเตือนทันทีในรอบ Polling ถัดไป ไม่มีหน่วงเวลา<br>2. **Priority Queue (xQueueSendToFront):** ข้อความเร่งด่วน (🚨 Critical / ⚠️ Warning / ❌ Offline) ถูกแทรกไปหัวคิว (Front) ส่งก่อนข้อความปกติเสมอ ข้อความปกติ (✅ Recovery / 📡 Online) ต่อท้ายคิวตามลำดับ<br>3. **FreeRTOS Task Priority:** ปรับ `TaskTelegram` จาก Priority 1 เป็น **Priority 2** (เท่ากับ LoRa Task) เพื่อให้ส่งข้อความแจ้งเตือนได้รวดเร็วไม่ถูก WebServer แย่ง CPU<br>4. **ไฟล์ที่เกี่ยวข้อง:** `TelegramNotifier.h`, `TelegramNotifier.cpp`, `main.cpp`, `README.md` |
| 2026-08-22 | `TX_Node` (Node 2) | **เปลี่ยนขาต่อ Ultrasonic Sensor (JSN-SR04T / HC-SR04) เป็น `TRIG = A3` และ `ECHO = A4`:**<br>1. ย้ายพิน Trigger มาใช้ **`A3`** และ Echo มาใช้ **`A4`** บนบอร์ด LoRa32u4 (ATmega32U4)<br>2. รองรับทั้งโปรไฟล์ `env:node2` (JSN-SR04T) และ `env:node3` (HC-SR04) สลับเซนเซอร์ได้ทันทีโดยใช้ตำแหน่งขาเดียวกัน<br>3. **ไฟล์ที่เกี่ยวข้อง:** `Config.h`, `JsnSr04tManager.cpp`, `HcSr04Manager.cpp`, `README.md` |
| 2026-08-22 | `RX_Master` | **ปรับปรุงการอัปเดตเกณฑ์ระดับน้ำ (Thresholds) ให้ Sync ข้อมูลแบบ Real-time 100%:**<br>1. **Live UI Input Feedback:** เมื่อพิมพ์เปลี่ยนค่าตัวเลขในช่อง เฝ้าระวัง หรือ วิกฤต ป้ายข้อความ `🟢 ปลอดภัย (> X cm) • 🟡 เฝ้าระวัง (≤ X cm) • 🔴 วิกฤต (≤ Y cm)` จะเปลี่ยนตามทันทีแบบ Real-time (`oninput`)<br>2. **Auto Server Sync:** หน้า Dashboard จะซิงค์ค่า Threshold ล่าสุดจาก Server ตลอดเวลาเมื่อไม่ได้กำลังพิมพ์ ไม่แสดงข้อมูลเก่าค้าง<br>3. **Instant Re-evaluation:** เมื่อกด "บันทึก" ฝั่ง ESP32 จะคำนวณสถานะ `floodStatus` และ % ความเสี่ยงใหม่ทันที ไม่ต้องรอรอบ Polling ถัดไป<br>4. **ไฟล์ที่เกี่ยวข้อง:** `SettingsManager.cpp`, `WebDashboard.h`, `README.md` |
| 2026-08-22 | `RX_Master` | **แก้ไขปัญหา Telegram Bot ไม่ตอบสนองหลังรันไปสักระยะ (Socket Exhaustion & TLS Cleanup Fix):**<br>1. **แก้ไข Socket Leak:** เพิ่มคำสั่ง `client.stop()` อย่างเคร่งครัดหลังจบทุก HTTPS Session ป้องกัน lwIP Socket Descriptor เต็ม (TIME_WAIT deadlock)<br>2. **แก้ไข Offset Initialization:** ปรับให้อ่าน `getUpdates?offset=-1&limit=1` ทันทีหลังบูต เพื่อดึง ID ล่าสุดและข้ามประวัติข้อความเก่าในอดีต (Backlog)<br>3. **ป้องกัน Heap Out of Memory:** เพิ่มการตรวจสอบหน่วยความจำ `ESP.getFreeHeap() < 25000 bytes` ก่อนดึง Updates เพื่อความเสถียรของ FreeRTOS Task<br>4. **ลด Timeout:** ปรับลด Handshake Timeout เหลือ 5 วินาที ไม่ให้บล็อกการทำงานของ Task<br>5. **ไฟล์ที่เกี่ยวข้อง:** `TelegramNotifier.cpp`, `README.md` |
| 2026-08-22 | ทั้งโปรเจกต์ | **ลบข้อมูลจำลองและชื่อจำลอง (Mockup Data & Fake Names) ออกทั้งหมด:**<br>1. **Master (`RX_Master`):** เปลี่ยนชื่อโหนดเริ่มต้นจากชื่อจำลอง ("จุดที่ 1 คลองระบายน้ำ", "จุดที่ 2 ริมแม่น้ำ") เป็น `Node 1` และ `Node 2` คลีน 100%<br>2. **Web Dashboard (`WebDashboard.h`):** ปรับชื่อหัวการ์ดและรายการตั้งค่าให้แสดงเฉพาะ `Node 1`, `Node 2` ไม่ให้มีข้อความ Mockup ซ้ำซ้อน<br>3. **Sensor Node (`TX_Node`):** ลบฟังก์ชัน Fallback จำลองคลื่น Sine Wave ออก เมื่อไม่มีเซนเซอร์จะคืนค่า 0 และสถานะ Fail จริง<br>4. **Telegram Notifier:** ปรับข้อความ Help และสถานะให้ใช้ชื่อ `Node #1`, `Node #2` สะอาดตา<br>5. **ไฟล์ที่เกี่ยวข้อง:** `RX_Master/src/main.cpp`, `WebDashboard.h`, `TelegramNotifier.cpp`, `TX_Node/src/FloodSensor.cpp`, `TX_Node/src/main.cpp`, `README.md` |
| 2026-08-22 | ทั้งระบบ (`RX_Master`) | **ปรับเกณฑ์เตือนภัยเป็นแบบเซนเซอร์ยิงจากบนลงล่าง (Top-Down Sensor) พร้อมเพิ่มระดับ "ปลอดภัย" (3 ระดับ):**<br>1. **ตรรกะใหม่ (Top-Down Clearance):** เซนเซอร์วัดระยะทางจากด้านบนลงผิวน้ำ (ระยะยิ่งน้อย = น้ำยิ่งสูง)<br>&nbsp;&nbsp;• 🟢 **ปลอดภัย (Safe):** ระยะห่าง $> \text{เฝ้าระวัง}$ (เช่น $> 300\text{ cm}$)<br>&nbsp;&nbsp;• 🟡 **เฝ้าระวัง (Warning):** $\text{วิกฤต} < \text{ระยะห่าง} \le \text{เฝ้าระวัง}$ (เช่น $200\text{ cm} < \text{ระยะห่าง} \le 300\text{ cm}$)<br>&nbsp;&nbsp;• 🔴 **วิกฤตน้ำท่วม (Critical):** $\text{ระยะห่าง} \le \text{วิกฤต}$ (เช่น $\le 200\text{ cm}$)<br>2. **แก้ไขการตรวจสอบเงื่อนไข (Validation):** กำหนดให้ค่าเกณฑ์ $\text{เฝ้าระวัง (Warn)} > \text{วิกฤต (Crit)} > 0$ เสมอ (เช่น Warn 300 cm, Crit 200 cm)<br>3. **ปรับปรุง Web Dashboard:** เพิ่มแถบแสดงระดับ **ปลอดภัย (Safe)**, แสดงช่วงระยะ 3 ระดับ พร้อมปรับปรุงป้ายสถานะและข้อความ Toast<br>4. **ปรับปรุง Telegram Notifier & OLED:** รองรับการแสดงผลสถานะ 3 ระดับ (🟢 ปลอดภัย / 🟡 เฝ้าระวัง / 🚨 วิกฤต) ทั้งในการแจ้งเตือนอัตโนมัติและคำสั่งบอท<br>5. **ไฟล์ที่เกี่ยวข้อง:** `Config.h`, `SettingsManager.cpp`, `LoRaEngine.cpp`, `WebPortal.cpp`, `WebDashboard.h`, `TelegramNotifier.cpp`, `DisplayManager.cpp`, `README.md` |
| 2026-08-22 | `RX_Master` | **เพิ่มระบบสั่งการและเรียกดูข้อมูลผ่าน Telegram Bot Commands (Interactive Bot):**<br>1. พัฒนาระบบประมวลผลคำสั่งแบบโต้ตอบ (Two-Way Interactive Bot Engine) ผ่าน Telegram Bot API `getUpdates`<br>2. รองรับชุดคำสั่งหลัก: `/status` (ดูภาพรวมทุกโหนด), `/node1` (ดูโหนด 1), `/node2` (ดูโหนด 2), `/poll` (สั่ง Polling ทันที), `/thresholds` (ดูเกณฑ์ระดับน้ำ), `/ip` (ดู Dashboard URL), `/help` (ดูเมนูคำสั่ง)<br>3. นำเข้า `ArduinoJson 7` สำหรับ Parse JSON แบบประหยัดหน่วยความจำและมีความปลอดภัยสูง<br>4. บอทสามารถตอบกลับได้ทั้งในกลุ่ม (Group Chat) และแชทส่วนตัว (Direct Message) ไปยังผู้สั่งการโดยตรง<br>5. ขยายขนาด FreeRTOS Task Stack (`TaskTelegram = 10240 bytes`) เพื่อรองรับ TLS + JSON Parser อย่างเสถียร<br>6. **ไฟล์ที่เกี่ยวข้อง:** `TelegramNotifier.h/cpp`, `main.cpp`, `platformio.ini`, `README.md` |
| 2026-08-22 | `RX_Master` | **แก้ไขปัญหา DNS Failed สำหรับ Telegram Bot API (`api.telegram.org`):**<br>1. กำหนด Public DNS Fallback (Google `8.8.8.8` & Cloudflare `1.1.1.1`) ป้องกันปัญหา Router บางรุ่น/เครือข่าย Hotspot ไม่แปลงชื่อโดเมนภายนอก<br>2. เพิ่มระบบ Retry Loop สูงสุด 3 ครั้ง พร้อมสลับใช้ Fallback DNS อัตโนมัติเมื่อเกิดข้อผิดพลาดในการ Resolve Hostname<br>3. เพิ่มการหน่วงเวลาก่อนส่งข้อความหลังเชื่อมต่อ WiFi ให้ TCP/IP Stack และ Routing พร้อมทำงาน 100%<br>4. เพิ่มข้อมูลแสดงผล Gateway และ DNS IP บน Serial Monitor เพื่อการวินิจฉัยปัญหาเครือข่ายอย่างแม่นยำ<br>5. **ไฟล์ที่เกี่ยวข้อง:** `TelegramNotifier.cpp`, `WebPortal.cpp`, `README.md` |
...
### ขั้นตอนที่ 4: การสั่งการและเรียกดูข้อมูลผ่าน Telegram Bot (Interactive Bot Commands)

ท่านสามารถพิมพ์คำสั่งในกลุ่ม Telegram หรือส่งข้อความส่วนตัว (Direct Message) หาบอท เพื่อเรียกดูข้อมูล Real-time หรือสั่งการได้ทันที:

| คำสั่ง (Command) | หน้าที่ / การทำงาน | ตัวอย่างผลลัพธ์ที่ตอบกลับ |
|---|---|---|
| **`/status`** หรือ `/data` | **ดูสถานะภาพรวมและระดับน้ำทุกโหนด:** แสดงระดับน้ำ (cm, %), สถานะเตือนภัย (Normal/Warn/Crit), คุณภาพสัญญาณ RSSI/SNR, Uptime และลิงก์ Dashboard | 📊 รายงานสถานะน้ำท่วม Node 1 (120 cm - ⚠️ เฝ้าระวัง), Node 2 (45 cm - ปกติ) |
| **`/node1`** หรือ `/1` | **ดูข้อมูลละเอียดเฉพาะ Node #1:** แสดงระดับน้ำ, เกณฑ์ตั้งค่า, ค่าเฉลี่ยแพ็กเกจที่รับได้, สัญญาณวิทยุ | 📍 ข้อมูลละเอียดจุดตรวจวัดคลองระบายน้ำ |
| **`/node2`** หรือ `/2` | **ดูข้อมูลละเอียดเฉพาะ Node #2:** แสดงระดับน้ำ, เกณฑ์ตั้งค่า, ค่าเฉลี่ยแพ็กเกจที่รับได้, สัญญาณวิทยุ | 📍 ข้อมูลละเอียดจุดตรวจวัดริมแม่น้ำ |
| **`/poll`** หรือ `/read` | **สั่งยิง LoRa Polling ทันที:** บังคับให้ Master ส่งคลื่นวิทยุเรียกอ่านค่าจาก Node ทุกตัวทันทีโดยไม่ต้องรอจังหวะลูปปกติ | 🔄 สั่งยิง LoRa Polling ไปยังทุกโหนดทันที |
| **`/thresholds`** หรือ `/set` | **ดูเกณฑ์ระดับน้ำเตือนภัย:** แสดงค่าเกณฑ์เฝ้าระวัง (Warn) และวิกฤต (Critical) ที่บันทึกอยู่ใน NVS Flash | ⚙️ Node #1 (Warn: 100cm, Crit: 200cm), Node #2 (...) |
| **`/ip`** หรือ `/dashboard` | **ดู IP Address และ URL ของ Web Dashboard:** แสดงลิงก์ IP, mDNS hostname และสัญญาณ WiFi | 🌐 http://10.197.119.81 / http://floodmonitor.local |
| **`/help`** หรือ `/start` | **แสดงเมนูคำสั่งทั้งหมด:** สรุปรายการคำสั่งที่บอทรองรับ | 🤖 เมนูแนะนำคำสั่งการใช้งาน |
...
|---|---|---|
| **TRIG** (Trigger) | **A3** | สัญญาณ Pulse เริ่มยิงคลื่น (Output) |
| **ECHO** (Echo) | **A4** | สัญญาณ Pulse สะท้อนกลับ (Input - pulseIn) |
